...
class user 
{
	...
	function add($firstname,$lastname,$email,$dob,$gender,$contact_no,$password,$imagefilename,$usertype,$ethinicity=0)
	{
		...
		$sql = "INSERT INTO `user` (`firstname`,`lastname`,`email`,`dob`,`gender`,`contact_no`,`password`,`ethinicity`,`photo`,`user_type`,`created_datetime`) VALUES
										('".$firstname."','".$lastname."','".$email."','".$dob."','".$gender."','".$contact_no."','".$password."','".$ethinicity."','".$photo."',$usertype,'".$now."')";
		$q = $DAO->query($sql);
		if($q)
		{
	...
	function login($username,$password,$access_token)
	{
	    ...
	    if(count($result)>0)
	    {
	    	...
	    	if($result[0]['user_type'] != 3 and $result[0]['is_deactive'] == 0)
			{
                ...
                // $descrypted_password = $result[0]['password'];
